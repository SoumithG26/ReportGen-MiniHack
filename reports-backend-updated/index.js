...
const buildReplacements = (body) => {
  ...
  return {
    ...
    "«below_than»": normalize(body.belowReading) || "below",
    "«Spelling_age»": normalize(body.spellingAge) || "",
    "«below_than1»": normalize(body.aboveSpelling) || "below",
    "«Reading_Discrepancy»": body.readingAge
      ? `This score is ${normalize(body.belowReading) || "below"} than ${pronouns.his_her} chronological age.`
      : "",
    "«Spelling_Discrepancy»": body.spellingAge
      ? `This score is ${normalize(body.aboveSpelling) || "below"} than ${pronouns.his_her} chronological age.`
      : "",
    "«NIMHANS_Section»": (() => {
      const tests = ((body.testsadministered || "") + " " + (body.otherTest || "")).toLowerCase();
      if (tests.includes("nimhans")) {
        return `<h4><b><u>NIMHANS SLD Index - Arithmetic test</u></b></h4>
      <p>
        This test is used for evaluating basic math skills of a school going child.
        In this simple math sums are given to check basic addition, subtraction,
        multiplication and division. Then the test provided graded level of all four
        math operations. At the end child shall attempt fractions and all operations of fractions.
        <span class="highlight">${body.name || ""} has difficulty with basic division and graded level of all
        four math operations and all fraction operations.</span>
      </p>`;
      }
      return "";
    })(),
    ...
    "«Informant»": body.informant === "other" && body.informantOther
      ? normalize(body.informantOther)
      : normalize(body.informant) || "",
  };
};
