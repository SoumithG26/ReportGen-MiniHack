...
router.post('/getAllTQScores', async (req, res) => {
  ...
  for (const test of tests) {
    ...
    try {
      ...
      if (!mapping) {
        // If raw score exceeds the max in the table, assign the highest possible TQ score
        const allScores = doc.mappings.map(m => m.raw_score);
        const maxRaw = Math.max(...allScores);
        if (Number(raw_score) > maxRaw) {
          const highestTQ = doc.mappings.reduce((best, m) => 
            m.tq_score > best.tq_score ? m : best, doc.mappings[0]);
          results.push({ name: normalizeCanonicalName(usedName), raw_score, tq_score: highestTQ.tq_score, capped: true });
        } else {
